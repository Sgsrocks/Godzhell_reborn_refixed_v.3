...
public abstract class Player {	
	...
	public Player(int _playerId) {
		...
		playerLook[0] = 0;//gender
		playerLook[1] = 7;//head
		playerLook[2] = 25;//torso
		playerLook[3] = 29;//arms
		playerLook[4] = 35;//hands
		playerLook[5] = 39;//legs
		playerLook[6] = 44;//feet
		playerLook[7] = 14; //beard
		playerLook[8] = 7; //hair color
		playerLook[9] = 8; //torso color
		playerLook[10] = 9; //legs color
		playerLook[11] = 5; //feet color
		playerLook[12] = 0; //skin color
	...
	protected void appendPlayerAppearance(stream str)
	{
		...
		if (isNpc == false) {
			  ...
			  if (!Item.isPlate(playerEquipment[playerChest])) {
			   playerProps.writeWord(0x100+playerLook[3]);
			  } else {
			   playerProps.writeByte(0);
			  }
			  ...
			  if (playerEquipment[playerHands] > 1) {
			   ...
			   playerProps.writeWord(0x100+playerLook[4]);
			  }
			  if (playerEquipment[playerFeet] > 1) {
			   playerProps.writeWord(0x200 + playerEquipment[playerFeet]);
			  } else {
			   playerProps.writeWord(0x100+playerLook[6]);
			  }
			if (!Item.isFullHelm(playerEquipment[playerHat]) && !Item.isFullMask(playerEquipment[playerHat]) && (playerLook[0] != 1))
			         playerProps.writeWord(0x100 + playerLook[7]);      // Beard without helm
			            else
			if (Item.isFullHelm(playerEquipment[playerHat]) && Item.isFullMask(playerEquipment[playerHat]) && (playerLook[0] != 1))
			         playerProps.writeWord(0x100 + playerLook[7]);      // Beard with helm
			            else
			  playerProps.writeByte(0);
			 } else {
		...
		playerProps.writeByte(playerLook[8]);	// hair color
		playerProps.writeByte(playerLook[9]);	// torso color.
		playerProps.writeByte(playerLook[10]);	// leg color
		playerProps.writeByte(playerLook[11]);	// feet color
		playerProps.writeByte(playerLook[12]);	// skin color (0-6)
