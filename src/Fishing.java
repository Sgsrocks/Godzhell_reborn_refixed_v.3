...
public class Fishing  {
		...
		for (int[] aData : data) {
			if (npcId == aData[0]) {
				if (c.playerLevel[c.playerFishing] < aData[1]) {
					...
					c.txt1("You need a fishing level of " + aData[1] + " to fish here.");
					return;
				}
				if (aData[3] > 0) {
					if (!c.playerHasItem(aData[3])) {
				...
				EventManager.getSingleton().addEvent(new Event() {
                    ...
                    public void execute(EventContainer container) {
						...
						if (c.fishingProp[2] > 0) {
                         c.addSkillXP(c.fishingProp[2], c.playerFishing);
						}
	...
	private static boolean hasFishingEquipment(client c, int equipment) {
		if (!c.playerHasItem(equipment)) {
			if(equipment == 311) {
				if(!c.playerHasItem(311) && !c.playerHasItem(10129) && c.playerEquipment[3] != 10129) {
